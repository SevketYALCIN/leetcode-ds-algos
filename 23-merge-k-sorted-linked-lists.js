var mergeKLists = function(lists) {
  let minPQ = new PriorityQueue("min")
  for(let i = 0; i < lists.length; i++) {
    while(lists[i]) {
      minPQ.insert(lists[i].val)
      lists[i] = lists[i].next
    }
  }
  
  let head   = new ListNode(Math.pow(-10, 5))
  let runner = head
  
  while(minPQ.heap.length >= 1) {
    runner.next = new ListNode(minPQ.extract())
    runner = runner.next
  }
